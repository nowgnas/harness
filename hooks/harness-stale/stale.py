...
설치는 하지 않는다(사용자 승인 후 harness install). 판단할 수 없으면 조용히 통과한다(fail-open).
"""
import json
import os
import subprocess
...
HARNESS = os.path.realpath(os.path.join(HOOK_DIR, "..", ".."))
STATUS_TIMEOUT = 8
...
GUIDE = ("하네스 설치본이 레포({repo})와 다릅니다 — 갱신 필요 {n}건:\n{items}\n"
         ...
         "`harness install --dry-run` → `harness install` 로 갱신하세요. 전체 상태는 `harness status`.")


def stale_lines(agents):
    """install.sh --status 출력에서 OK 가 아닌 줄을 모은다."""
    proc = subprocess.run([os.path.join(HARNESS, "install.sh"), "--status", "--global",
                           "--agents", ",".join(agents)],
