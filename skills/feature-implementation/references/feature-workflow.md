...
## 8. 리뷰
1. 셀프 정리: 전체 diff를 다시 읽고 디버그 코드, 사용하지 않는 코드, 불필요한 주석, 설계 밖 변경을 걷어낸다.
2. 독립 리뷰: `impl-review` 스킬로 구현 맥락이 없는 리뷰어에게 리뷰받는다. 🔴·🟠 지적은 반영하고 7단계 검증을 다시 실행한다. 반영하려면 설계나 정책을 바꿔야 하면 멈추고 묻는다.
