...
```bash
git clone https://github.com/nowgnas/harness.git ~/harness
~/harness/install.sh --dry-run    # 무엇이 바뀌는지 확인
~/harness/install.sh              # 글로벌 적용
harness doctor                    # 점검
harness help                      # 사용법
```
...
| 서브에이전트 | `agents/*.md` (`task:`) | `~/.claude/agents/*.md` 생성 (`model:` 포함) | `config.toml`의 `[agents.<이름>]` 역할 + `~/.codex/harness/agents/*.toml` |
...
| `harness-manage` | 스킬·룰·훅 조회·추가·설치 | "이 스킬 설치해줘" |
| `harness-help` | 사용법 안내 ([USAGE.md](skills/harness-help/USAGE.md)) | "하네스 사용법 알려줘" |

## 토큰을 아끼는 조회

에이전트가 문서를 통째로 읽지 않도록, 조회는 `lib/harness.py`가 짧게 잘라서 돌려줍니다.

```bash
harness list                                        # 항목과 설치 상태 (한 줄씩)
harness list skills --all                           # 외부·플러그인 스킬까지 → 중복 확인
harness show skill feature-implementation --toc     # 목차와 섹션별 줄 수
harness show skill feature-implementation --section 5
harness show ref db-migration --section "락"
harness search "expand" -l
```
...
```bash
harness add skill https://github.com/owner/repo.git --subdir skills/foo   # 외부 스킬 추가
harness new skill my-skill | new rule team-style | new hook my-hook       # 뼈대 생성
harness adopt skill dev-guide                                             # 외부 스킬을 하네스로 이동
harness hook disable guard-agent-config                                   # 훅 끄기
harness install --dry-run && harness install                              # 적용
```
...
lib/harness.py          조회·관리·훅·모델 동기화 (python3 표준 라이브러리)
lib/usage.py            세션 로그 토큰 집계 (harness usage)
lib/bench.py            작업 세트 조건별 실행·비교 (harness bench)
bench/                  벤치 작업 예시
bin/harness             CLI
install.sh              설치/상태/제거
...
```bash
harness update       # git pull + 재설치
harness uninstall    # 하네스가 만든 링크·블록·훅만 제거
```
